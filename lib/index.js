...
/**
 * promiseAllEnd reject data is a mix type like `{error, data}`.
 * use decoreatePromise mix type will been separated
 * PS. `Promise.prototype.catch` directly call `Promise.prototype.then` so we just need decorate then.
 * @param  {Promise} promise before decorate Promise object
 * @return {Promise}         decorated Promise object
 */
function decoratePromise(promise) {
  promise.then = function (handleFulfilled, handleRejected) {
    let handleRejectedDecorated = decorateThenRejected(handleFulfilled, handleRejected);
    let nextPromise = promiseThen.call(promise, handleFulfilled, handleRejectedDecorated);
...
function decorateThenRejected(handleFulfilled, handleRejected) {
  return !handleRejected ?
    function ({error, data}) {
      let rejectValueMix = {error};
      rejectValueMix.data = callFulfilled(handleFulfilled, data);
      return Promise.reject(rejectValueMix);
    }
    :
    function ({error, data}) {
      callFulfilled(handleFulfilled, data);
      return handleRejected(error);
    }
}

function callFulfilled(handleFulfilled, data) {
  // 如果有注册了 fullfilled 事件，则计算新的 data 并传递
  return data && handleFulfilled && handleFulfilled(data);
}
