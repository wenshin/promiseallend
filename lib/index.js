...
function promiseAllEnd(promises, options = {}) {
  ...
  function onPendingFinish(result, resolve, reject) {
    ...
    function _handleRestRejected() {
      if (result.errorKeys.length && options.unhandledRejection) {
        process.nextTick(() => {
          for (const key of result.errorKeys) {
            key !== result.rejectedKey
              && unhandledRejection.call(promise, result.errorsByKey[key], key);
          }
        });
      }
    }
...
function newRjectedError(result, key) {
  let err = new Error(`promiseAllEnd rejected promises in ${result.errorKeys}`);
  if (key) {
    err.detail = result.errorsByKey[key];
    err.key = key
  } else {
