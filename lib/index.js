    ...
    function rejectedWrap(key) {
      return error => {
        if (error.errorsByKey) {
          Object.keys(error.dataByKey).length && (result.dataByKey[key] = error.dataByKey);
...
function decoratePromise(promise) {
  ...
  promise.then = function(handleFullfilled, handleRejected) {
    let newHandleRejected = errorData => {
      errorData.data && handleFullfilled(errorData.data);
      return handleRejected ? handleRejected(errorData.error) : Promise.reject(errorData.error);
    }
